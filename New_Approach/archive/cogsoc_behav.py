...
MODEL_PATH       = "/mnt/c/CogSOC/Trained AI/better_model_v3.pkl"
SCALER_PATH      = "/mnt/c/CogSOC/Trained AI/better_scaler_v3.pkl"
BLOCKED_LOG      = "/mnt/c/CogSOC/data/blocked_ips.log"
...
def _res_path(wsl_path, win_path):
    return win_path if os.path.exists(win_path) else wsl_path

CICFLOW_OUTPUT_DIR = _res_path("/mnt/c/CogSOC/CICFlowMeter-master/build/distributions/CICFlowMeter-4.0/bin/data/daily", r"C:\CogSOC\CICFlowMeter-master\build\distributions\CICFlowMeter-4.0\bin\data\daily")
...
CICFLOW_BIN = _res_path("/mnt/c/CogSOC/CICFlowMeter-master/build/distributions/CICFlowMeter-4.0/bin/cfm.bat", r"C:\CogSOC\CICFlowMeter-master\build\distributions\CICFlowMeter-4.0\bin\cfm.bat")
...
LIVE_CHUNK_SECONDS = int(os.environ.get("COGSOC_LIVE_CHUNK_SECONDS", "30"))

FILEBEAT_EXE = _res_path("/mnt/c/Program Files/Elastic/Beats/9.3.3/filebeat/filebeat.exe", r"C:\Program Files\Elastic\Beats\9.3.3\filebeat\filebeat.exe")
FILEBEAT_CONFIG = _res_path("/mnt/c/Program Files/Elastic/Beats/9.3.3/filebeat/filebeat.yml", r"C:\Program Files\Elastic\Beats\9.3.3\filebeat\filebeat.yml")
FILEBEAT_FALLBACK_CONFIG = _res_path("/mnt/c/CogSOC/Setup/filebeat-windows.yml", r"C:\CogSOC\Setup\filebeat-windows.yml")
FILEBEAT_LOG = os.path.join(CICFLOW_OUTPUT_DIR, "filebeat.log")
TSHARK_EXE = _res_path("/mnt/c/Program Files/Wireshark/tshark.exe", r"C:\Program Files\Wireshark\tshark.exe")
...
def ensure_live_flow_dir():
    """Ensure the live CICFlowMeter output folder exists."""
    os.makedirs(CICFLOW_OUTPUT_DIR, exist_ok=True)


def to_win_path(path):
    """Convert a WSL path (/mnt/c/...) to a Windows path (C:/) for host tools."""
...
def get_cicflow_command(input_path, output_dir):
    """Return the offline CICFlowMeter command for a pcap file/folder."""
    # Always prefer the Python version since the Java one has jnetpcap DLL issues on this host
    if os.path.exists(CICFLOW_PYTHON_BIN):
        win_in = to_win_path(input_path)
        # Python cicflowmeter expects the exact output CSV file, not a directory
        # If output_dir is provided, we create a file name based on the input
        base_name = os.path.basename(input_path) + "_Flow.csv"
        win_out = to_win_path(os.path.join(output_dir, base_name))
        return [CICFLOW_PYTHON_BIN, "-f", win_in, "-c", win_out]
        
    # Fallback to Java version
    if os.path.exists(CICFLOW_BIN):
        return [CICFLOW_BIN, input_path, output_dir]
    cfm = shutil.which("cfm") or shutil.which("cicflowmeter") or shutil.which("cfm.bat")
    if cfm:
        return [cfm, input_path, output_dir]
    return None


def get_packet_capture_command(pcap_path, seconds=LIVE_CHUNK_SECONDS):
    """Build a bounded packet capture command for live automation."""
    # From WSL, we can call the Windows tshark.exe to capture host Wi-Fi
    if os.path.exists(TSHARK_EXE):
        win_pcap = to_win_path(pcap_path)
        return [
            TSHARK_EXE, "-i", CICFLOW_INTERFACE, "-a", f"duration:{seconds}",
            "-w", win_pcap
        ]
    if shutil.which("tcpdump"):
        return [
            "sudo", "tcpdump", "-i", CICFLOW_INTERFACE, "-nn", "-s", "0",
...
def run_cicflowmeter(input_path, output_dir):
    """Run CICFlowMeter and return generated CSV files."""
    os.makedirs(output_dir, exist_ok=True)
    cmd = get_cicflow_command(input_path, output_dir)
    if not cmd:
        raise RuntimeError("CICFlowMeter command not found. Update CICFLOW_BIN or PATH.")
    ...
    # Translate paths for Windows .bat/.exe
    # Note: get_cicflow_command already translates paths for the python version
    if cmd[0] == CICFLOW_PYTHON_BIN:
        cmd_to_run = cmd
    else:
        win_input = to_win_path(input_path)
        win_output = to_win_path(output_dir)
        cmd_to_run = [cmd[0], win_input, win_output] if len(cmd) > 1 else cmd

    print(f"[CICFlowMeter] Running: {' '.join(cmd_to_run)}")
    subprocess.run(cmd_to_run, cwd=os.path.dirname(CICFLOW_BIN), check=True, timeout=900)
    after = {
        os.path.join(output_dir, name)
        for name in os.listdir(output_dir)
        if name.lower().endswith(".csv")
    ...
    if not generated:
        generated = sorted(after, key=os.path.getmtime)[-1:]
    return generated


def live_cicflow_loop(stop_after_seconds=None):
    """Capture short PCAP chunks and convert each chunk to CICFlowMeter CSV."""
    ensure_live_flow_dir()
    os.makedirs(LIVE_PCAP_DIR, exist_ok=True)
    started = time.time()
    print(f"[CAPTURE] Live interface: {CICFLOW_INTERFACE}")
    print(f"[CAPTURE] Chunk size: {LIVE_CHUNK_SECONDS}s")

    while True:
        if stop_after_seconds and time.time() - started >= stop_after_seconds:
            print("[CAPTURE] Live capture duration reached. Stopping capture loop.")
            return

        stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        pcap_path = os.path.join(LIVE_PCAP_DIR, f"live_{stamp}.pcap")
        capture_cmd = get_packet_capture_command(pcap_path)
        if not capture_cmd:
            print("[CAPTURE] ERROR: tcpdump/tshark not found. Install one in WSL for automated live capture.")
            return

        try:
            print(f"[CAPTURE] Capturing chunk: {' '.join(capture_cmd)}")
            subprocess.run(capture_cmd, check=False, timeout=LIVE_CHUNK_SECONDS + 15)
            if not os.path.exists(pcap_path) or os.path.getsize(pcap_path) == 0:
                print("[CAPTURE] Empty PCAP chunk; check interface name/permissions.")
                time.sleep(2)
                continue

            csvs = run_cicflowmeter(pcap_path, CICFLOW_OUTPUT_DIR)
            print(f"[CICFlowMeter] Generated {len(csvs)} CSV file(s): {', '.join(os.path.basename(p) for p in csvs)}")
        except subprocess.TimeoutExpired:
            print("[CAPTURE] Packet capture timed out; starting next chunk.")
        except Exception as e:
            print(f"[CAPTURE] Live chunk failed: {e}")
            time.sleep(5)


def start_cicflow_capture(stop_after_seconds=None):
    """Start automated live capture using Python CICFlowMeter in the background."""
    ensure_live_flow_dir()
    
    # We prefer the Python version for live capture as it's more stable on Windows/WSL hybrid
    if os.path.exists(CICFLOW_PYTHON_BIN):
        win_out = to_win_path(CICFLOW_OUTPUT_FILE)
        cmd = [CICFLOW_PYTHON_BIN, "-i", CICFLOW_INTERFACE, "-c", win_out]
        print(f"[CAPTURE] Starting Python CICFlowMeter: {' '.join(cmd)}")
        
        try:
            logf = open(CICFLOW_LOG, "a")
            proc = subprocess.Popen(cmd, stdout=logf, stderr=logf)
            ACTIVE_PROCESSES.append(proc)
            return proc
        except Exception as e:
            print(f"[CAPTURE] Error starting Python CICFlowMeter: {e}")
            return None

    # Fallback to the old pcap-loop method if python tool is missing
    print("[CAPTURE] WARNING: Python cicflowmeter.exe not found. Falling back to PCAP chunk loop.")
...
def get_filebeat_config():
    """Return a valid Filebeat config path, generating one on-the-fly if needed."""
    # Prefer the user-provided fallback (written by dashboard._ensure_filebeat_config)
    ...
    # ── Auto-generate a minimal config so Filebeat can always start ──
    setup_dir = os.path.dirname(FILEBEAT_FALLBACK_CONFIG)
    try:
        os.makedirs(setup_dir, exist_ok=True)
    except Exception:
        setup_dir = CICFLOW_OUTPUT_DIR  # last resort: write next to data

    generated_path = os.path.join(setup_dir, "filebeat-cogsoc-autogen.yml")
    config_content = f"""# Auto-generated by CogSOC cogsoc_behav.py  (interface: {CICFLOW_INTERFACE})
filebeat.inputs:
  - type: log
    enabled: true
    paths:
      - "{CICFLOW_OUTPUT_DIR}/*.csv"
      - "{LIVE_PCAP_DIR}/*.csv"
    fields:
      cogsoc_source: cicflowmeter
      capture_interface: "{CICFLOW_INTERFACE}"
...
"""
    try:
        ...
        print(f"[FILEBEAT] Auto-generated config → {generated_path}  (index: cogsoc-flows)")
        return generated_path
    except Exception as e:
        print(f"[FILEBEAT] ERROR: Could not write auto-generated config: {e}")
        return None


def start_filebeat():
    """Start Filebeat with the configured pipeline."""
    ensure_live_flow_dir()
    config_path = get_filebeat_config()
    filebeat_exe = FILEBEAT_EXE
    if not os.path.exists(filebeat_exe) and os.path.exists(f"{filebeat_exe}.exe"):
        filebeat_exe = f"{filebeat_exe}.exe"
    if config_path is None:
        print("[FILEBEAT] WARNING: No Filebeat config found. Update FILEBEAT_CONFIG or FILEBEAT_FALLBACK_CONFIG.")
        return None
    if not os.path.exists(filebeat_exe):
        print(f"[FILEBEAT] WARNING: Filebeat executable not found at {FILEBEAT_EXE}.")
        return None

    logfile = open(FILEBEAT_LOG, "a", encoding="utf-8", errors="ignore")
    cmd = [filebeat_exe, "-c", config_path, "-e"]
    print(f"[FILEBEAT] Starting Filebeat: {' '.join(cmd)}")
    try:
...
class IncidentCorrelationEngine:
    ...
    def ingest_alert(self, src_ip, dst_ip, flow_doc, ai_conf, attack_type,
                     severity, sensor, mitre_t, mitre_tac, blocked):
        """
        ...
        if src_ip in self.incidents:
            ...
            # Accumulate flow evidence
            inc['evidence_flows'].append(flow_doc)
            if len(inc['evidence_flows']) > 100:
                inc['evidence_flows'] = inc['evidence_flows'][-100:]
...
def fetch_flows(es, last_ts, size=BATCH_SIZE):
    ...
    # ── Strategy: use search_after pagination with _doc sort ──
    # This works for ALL data (with or without @timestamp)
    try:
        query = {
            "query": {"match_all": {}},
            "size": size,
        ...
        if not hits:
            # All docs exhausted — check for NEW docs (real-time Filebeat)
            # Reset cursor and try @timestamp range for live data
            try:
                live_query = {
                    "query": {"range": {"@timestamp": {"gte": last_ts, "lte": "now"}}},
                    "size": size,
                    "sort": [{"@timestamp": {"order": "asc", "unmapped_type": "date"}}]
                }
                live_resp = es.search(index=f"{FLOWS_INDEX}*", **live_query)
                live_hits = live_resp['hits']['hits']
                if live_hits:
                    new_hits = [h for h in live_hits if h['_id'] not in _seen_ids]
                    if new_hits:
                        new_hits = new_hits[:size]
                        rows = [h['_source'] for h in new_hits]
                        ids  = [h['_id']     for h in new_hits]
                        df   = pd.DataFrame(rows)
                        new_ts = new_hits[-1]['_source'].get('@timestamp', last_ts)
...
def save_alert(es, alert):
    try:
        es.index(index=ALERTS_INDEX, document=alert)
    except Exception as e:
...
def run_monitor(server_mode=False, duration_minutes=None):
    ...
    last_ts        = (datetime.now(timezone.utc) - timedelta(days=30)).isoformat()  # pick up old + new data
    total_analyzed = 0
    total_alerts   = 0
    total_blocked  = 0
    total_skipped  = 0
        ...
        while True:
            try:
                ...
                for ip in batch_correlated_ips:
                    ...
                    if inc:
                        # THE HANDS: Evaluate playbooks for this incident
                        if playbook_engine:
                            playbook_engine.evaluate(inc)

                        # THE HANDS: Send email alert if warranted
                        if email_engine:
    ...
    for proc in ACTIVE_PROCESSES:
        try:
            if proc.poll() is None:
                proc.terminate()
                print(f"[SHUTDOWN] Killed process: {proc.pid}")
        except Exception:
            pass
        ...
        if args.mode == 'kibana-guide':
            ...
            if not args.engine_only:
                ...
                if args.offline_file:
                    ...
                    # Live mode: start CICFlowMeter + Filebeat pipeline automatically
                    try:
